...
Write-Host "   黄历网站 - Vercel 部署工具" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# 检查 Vercel CLI 是否已安装
Write-Host "正在检查 Vercel CLI..." -ForegroundColor Yellow
try {
...
# 提示用户登录
Write-Host "步骤 1: 登录 Vercel" -ForegroundColor Yellow
Write-Host "提示: 如果已登录，可以跳过此步骤" -ForegroundColor Gray
Write-Host ""
$login = Read-Host "是否需要登录 Vercel? (y/n)"
...
Write-Host "提示: 按照提示选择配置选项" -ForegroundColor Gray
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "========================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "你的网站已成功部署到 Vercel！" -ForegroundColor White
    ...
    Write-Host "- 可以在 Vercel 控制台管理你的项目: https://vercel.com/dashboard" -ForegroundColor White
    Write-Host ""
}
else {
    Write-Host ""
