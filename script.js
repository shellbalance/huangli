...
class LunarCalendar {
    constructor() {
        ...
        this.pengZuZhi = {
            ...
            "申": "申不安床鬼祟入房", "酉": "酉不会客醉坐禾张", "戌": "戌不吃犬作怪上床", "亥": "亥不嫁娶不利新郎"
        };

        // 二十八星宿 (参考: 2019-01-17 为 角木蛟)
        this.stars28 = [
            { name: "角木蛟", luck: "吉", info: "角星造作主荣昌，外进田财及女郎。" },
            { name: "亢金龙", luck: "凶", info: "亢星造作长房当，十日之中主有殃。" },
            { name: "氐土貉", luck: "凶", info: "氐星造作主灾凶，费尽田园仓库空。" },
            { name: "房日兔", luck: "吉", info: "房星造作田园进，钱财牛马遍山岗。" },
            { name: "心月狐", luck: "凶", info: "心星造作大为凶，更糟刑讼狱囚中。" },
            { name: "尾火虎", luck: "吉", info: "尾星造作主天恩，富贵荣华福寿宁。" },
            { name: "箕水豹", luck: "吉", info: "箕星造作主高强，岁岁年年大吉昌。" },
            { name: "斗木蟹", luck: "吉", info: "斗星造作主招财，文武官员位上排。" },
            { name: "牛金牛", luck: "凶", info: "牛星造作主灾危，九横三灾不可推。" },
            { name: "女土蝠", luck: "凶", info: "女星造作损婆娘，兄弟相嫌似虎狼。" },
            { name: "虚日鼠", luck: "凶", info: "虚星造作主灾殃，男女孤眠不一双。" },
            { name: "危月燕", luck: "凶", info: "危星造作主天刑，遭官失盗产儿亡。" },
            { name: "室火猪", luck: "吉", info: "室星造作进田牛，儿孙代代近王侯。" },
            { name: "壁水貐", luck: "吉", info: "壁星造作主增财，丝蚕大熟福滔天。" },
            { name: "奎木狼", luck: "凶", info: "奎星造作得祯祥，家内荣和大吉昌。" },
            { name: "娄金狗", luck: "吉", info: "娄星造作起门庭，财旺家和事事兴。" },
            { name: "胃土雉", luck: "吉", info: "胃星造作事如何，富贵荣华喜气多。" },
            { name: "昴日鸡", luck: "凶", info: "昴星造作进田牛，埋葬官灾不得休。" },
            { name: "毕月乌", luck: "吉", info: "毕星造作主光前，买得田园有粟钱。" },
            { name: "觜火猴", luck: "凶", info: "觜星造作有徒刑，三年必定主伶仃。" },
            { name: "参水猿", luck: "吉", info: "参星造作旺人家，文星照耀大光华。" },
            { name: "井木犴", luck: "吉", info: "井星造作旺蚕田，金榜题名第一先。" },
            { name: "鬼金羊", luck: "凶", info: "鬼星起造卒人亡，堂前不见主人郎。" },
            { name: "柳土獐", luck: "凶", info: "柳星造作主遭官，昼夜偷闲不暂安。" },
            { name: "星日马", luck: "凶", info: "星星造作不如房，进得三牛九口粮。" }, // Note: Typically negative in some contexts but rhyme varies. Using standard.
            { name: "张月鹿", luck: "吉", info: "张星造作主荣昌，文武相逢七代郎。" },
            { name: "翼火蛇", luck: "凶", info: "翼星造作主天孤，岁岁朝朝在路途。" },
            { name: "轸水蚓", luck: "吉", info: "轸星造作主增财，朝仁列宿照无灾。" }
        ];
    }
    ...
    getPengZu(ganzhi) {
        const gan = ganzhi[0];
        const zhi = ganzhi[1];
        return (this.pengZuGan[gan] || "") + " " + (this.pengZuZhi[zhi] || "");
    }

    // 获取二十八星宿
    get28Star(date) {
        const baseDate = new Date(2019, 0, 17); // 2019-01-17 acts as base (Index 0: Jiao)
        const offset = Math.floor((date - baseDate) / 86400000);
        let index = offset % 28;
        if (index < 0) index += 28;
        return this.stars28[index];
    }
}
...
class HuangLiApp {
    ...
    updateExtraInfo() {
        ...
        // 煞方位：根据日支确定
        // 煞方位：三合五行推算 (申子辰水局煞南, 亥卯未木局煞西, 寅午戌火局煞北, 巳酉丑金局煞东)
        const shaDirections = ['南', '东', '北', '西', '南', '东', '北', '西', '南', '东', '北', '西'];
        const shaDirection = shaDirections[dayZhiIndex];

        document.getElementById('chongsha').textContent = `冲${chongAnimal}煞${shaDirection}`;

        // 彭祖百忌
        const pengzu = this.lunar.getPengZu(ganzhiDay);
        document.getElementById('pengzu').textContent = pengzu;

        // 二十八星宿
        const star = this.lunar.get28Star(this.currentDate);
        document.getElementById('star28').textContent = `${star.name} (${star.luck})`;
        document.getElementById('star28Info').textContent = star.info;
        ...
        const wealthGod = this.getWealthGod(dayGan);
        const joyGod = this.getJoyGod(dayGan);
        const luckGod = this.getLuckGod(dayGan);

        const godsInfo = document.getElementById('godsDirections');
        if (godsInfo) {
            godsInfo.textContent = `财神${wealthGod} · 喜神${joyGod} · 福神${luckGod}`;
        }
    }
